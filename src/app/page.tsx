...
export async function generateMetadata({ searchParams }: { searchParams: Promise<{ lang?: string }> }): Promise<Metadata> {
  ...
  return {
    title: `GMOPlus Jobs Blog${langLabel}`,
    description: "Kariyer haberleri, iş arama rehberleri ve mülakat ipuçları",
    alternates: {
...
function PostCard({ post, lang }: { post: any; lang?: string }) {
  return (
    <Link href={`/${post.slug}`} className="group block">
      ...
      <h2 className="font-bold text-gray-900 group-hover:text-violet-600 transition-colors line-clamp-2 mb-1">{post.title}</h2>
    ...
    <div className="max-w-5xl mx-auto px-4 py-10">
      {heroPost && (
        <Link href={`/${heroPost.slug}`} className="group block mb-12">
          <div className="relative rounded-2xl overflow-hidden bg-violet-900">
            {heroPost.coverImageUrl ? (
              ...
              <div className="w-full h-80 bg-gradient-to-br from-violet-700 to-violet-900" />
            )}
            <div className="absolute inset-0 p-8 flex flex-col justify-end">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-semibold text-violet-200 bg-violet-800/60 px-2.5 py-1 rounded-full backdrop-blur-sm">
                  KARİYER
                </span>
              ...
              {heroPost.excerpt && <p className="text-sm text-violet-100 line-clamp-2">{heroPost.excerpt}</p>}
              <p className="text-xs text-violet-200 mt-3">{formatDate(heroPost.publishedAt, lang)} · {heroPost.readingTimeMin} dk</p>
            </div>
          </div>
        </Link>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {result?.hasMore && result?.nextCursor && (
            <div className="mt-10 text-center">
              <Link
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 bg-violet-600 text-white text-sm font-medium rounded-xl hover:bg-violet-700 transition-colors"
              >
                Daha Fazla Yükle
              </Link>
            </div>
        ...
        <aside className="space-y-8">
              ...
              <div className="space-y-4">
                {popular.map((p: any, i: number) => (
                  <Link key={p._id} href={`/${p.slug}`} className="flex gap-3 group">
                    <span className="text-2xl font-black text-violet-100 w-6 shrink-0">{i + 1}</span>
                    <div>
                      <h4 className="text-sm font-semibold text-gray-800 group-hover:text-violet-600 transition-colors line-clamp-2">{p.title}</h4>
          ...
          <div className="bg-violet-50 rounded-xl p-5">
            <h3 className="text-sm font-bold text-violet-900 mb-2">Jobs platformuna gidin</h3>
            <p className="text-xs text-violet-700 mb-3">İş ilanları, kariyer fırsatları ve daha fazlası.</p>
            <a href="https://jobs.gmoplus.com" className="inline-flex items-center gap-1.5 text-xs font-semibold text-violet-600 hover:text-violet-700">
              jobs.gmoplus.com →
            </a>
          </div>
        </aside>
