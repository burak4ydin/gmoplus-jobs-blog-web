...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    if (!author) return { title: "Yazar — GMOPlus Jobs Blog" };
    return {
      title: `${author.displayName} — GMOPlus Jobs Blog`,
      description: author.bio || `${author.displayName} tarafından yazılan makaleler.`,
    };
  } catch {
    return { title: "Yazar — GMOPlus Jobs Blog" };
  }
}
      ...
      {author && (
        <header className="mb-8 flex flex-col sm:flex-row items-start gap-5">
          {author.avatarUrl ? (
            ...
            <div className="w-20 h-20 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center text-2xl font-bold shrink-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-10">
          {data.items.map((post: any) => (
            <article key={post.id} className="group bg-white rounded-xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-5">
                <a href={`/${post.slug}`}>
                  <h2 className="text-base font-bold text-gray-900 group-hover:text-teal-600 transition-colors line-clamp-2 mb-2 leading-snug">
                    {post.title}
                  </h2>
                </a>
                {post.excerpt && (
